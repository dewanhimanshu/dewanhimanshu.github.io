...
const Experience = () => {
  ...
  return (
    <section className="py-24 bg-gray-50/30 dark:bg-gray-800/30" id="experience">
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="space-y-6">
          {experiences.map((exp, index) => (
            <Card key={index} className="border border-gray-100/50 dark:border-gray-700/50 bg-white dark:bg-gray-800/50 rounded-3xl overflow-hidden hover:shadow-sm transition-all duration-300">
              <CardContent className="p-8">
                <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 mb-6">
                  <div className="flex-1">
                    <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">{exp.title}</h3>
                    <p className="text-lg text-gray-700 dark:text-gray-300 font-medium mb-1">{exp.company}</p>
                    <p className="text-gray-500 dark:text-gray-400 text-sm">{exp.location}</p>
                  </div>
                  <div className="text-right">
                    <span className="inline-block bg-gray-50 dark:bg-gray-700 text-gray-700 dark:text-gray-300 px-4 py-2 rounded-full text-sm font-medium border border-gray-100 dark:border-gray-600">
                      {exp.period}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {exp.technologies.map((tech, techIndex) => (
                    <span 
                      key={tech} 
                      className="bg-gray-50 dark:bg-gray-700 text-gray-600 dark:text-gray-300 px-3 py-1 rounded-full text-sm font-medium border border-gray-100 dark:border-gray-600"
                    >
                      {tech}
                    </span>
                  ))}
                ...
                <ul className="space-y-3">
                  {exp.achievements.map((achievement, idx) => (
                    <li key={idx} className="text-gray-600 dark:text-gray-300 flex items-start text-sm leading-relaxed">
                      <span className="w-1.5 h-1.5 bg-gray-400 dark:bg-gray-500 rounded-full mt-2 mr-4 flex-shrink-0"></span>
